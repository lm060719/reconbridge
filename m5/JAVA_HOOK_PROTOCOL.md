...
Phase 4 把 Android 生命周期和可用 Context 变成 Runtime 的一等输入。目标进程会创建 `ContextRegistry` 与 `LifecycleManager`：

- Hook `Application.attach(Context)`，并尝试用 `ActivityThread.currentApplication()` 补当前 Application；
- 获得 Application 后注册官方 `Application.ActivityLifecycleCallbacks`；
- 当前 Activity 始终使用 **WeakReference**，不会因为 Tracer 监控而延长页面生命周期；
- 生命周期回调通过现有 Event Bus 发出 `lifecycle.*` 事件，因此可以直接驱动 Phase 3 的 State / Event → Action；
- runtime status 的生命周期刷新在后台合并发送，避免在 UI 主线程同步写整份状态快照。

### Action / 模板可直接使用的 Runtime 对象
...
模板、condition、`call_method.target`、`set_field.target` 等继续复用同一个路径解析器：
...
```jsonc
{
  ...
  "scope": "process",
  "key": "screen",
  "value": "${lifecycle.activity_class}:${lifecycle.activity_state}"
}
```
...
当当前 Activity 不存在时，`activity.*` 返回 unresolved/MISSING；`context` 会按：
...
`activity_created` 还会带 `saved_state_present`，`activity_save_instance_state` 会带 `bundle_size`。

### on_lifecycle 语法

`kind:"runtime"` target 或普通 Java target 都可以使用 `on_lifecycle`。它最终复用 Event Bus，但语法比手写 `on_event` 更短：

```jsonc
{
  "kind": "runtime",
  "id": "vip_screen_runtime",
  "on_lifecycle": {
    "stage": "resumed",
    "activity": "VipActivity",
    "actions": [
...
也可以直接写完整事件名，例如 `lifecycle.activity_resumed`。

Activity 过滤支持：

```jsonc
{"activity":"com.foo.VipActivity"}
{"activity":"VipActivity"}
{"activity_match":"foo\\..*Activity$"}
```

`activity` 支持完整类名和简单类名；`activity_match` 使用正则。普通 `on_event` 不会被这些生命周期过滤规则误影响。

### Runtime Status

`runtime_hook_status(package)` 新增：

```jsonc
{
  "context_runtime": {
    "application_available": true,
    "application_class": "com.foo.App",
    "context_available": true,
    "activity_available": true,
    "activity_class": "com.foo.VipActivity",
    "activity_state": "resumed",
...
注意：
- Lifecycle Runtime 是**进程内**的；多进程 App 每个已连接进程各有自己的 Application/Activity 状态。
- 当前 Activity 不是强引用；页面销毁后不会因为 Tracer 自己被永久保留。
- Compose/Fragment/自绘视图不是独立 Activity 生命周期。如果要判断具体 Fragment/Composable 状态，仍应 Hook 对应业务方法或自行 emit 自定义事件。
- `on_lifecycle` 是 Activity/Application 级触发器，不替代系统级 Zygote/Resource Hook；后者仍属于后续 Runtime 阶段。
