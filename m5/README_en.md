...
Supports Trace (observation), live add/remove/replace, true live unhook, **pending hooks with dynamic ClassLoader watching**, real-time tampering (arg/return replacement, skip original, field & Map key mutation `mutate_return`), conditional execution (`condition`/`if`), path-based return access in `after` phase, **Action Pipeline** (invoke Java methods, read/write private fields, instantiate complex objects, evaluate Rhino JS / DEX snippets, execute shell commands), and template variables `${...}`. Explicit class targets that are not visible from any known loader enter `pending_class` and are retried when Path/Dex/InMemoryDexClassLoader instances appear or when the target class is returned by `ClassLoader.loadClass`. Requires LSPosed with scope enabled. See `JAVA_HOOK_PROTOCOL.md` for details.
