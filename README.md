...
自动下载打包好的 MCP exe、解压到 `%LOCALAPPDATA%\ReconBridge\`、注册进 Claude Code 用户级配置，并铺一个 `reconbridge` skill 到 `~/.claude/skills/`（逆向类任务时新会话自动加载工作流）。装完**重启 Claude Code** 即用。
...
cd reconbridge
./install.ps1              # 建 venv → 装依赖 → 注册 reconbridge + 铺 skill 到 Claude Code（用户级）
```
...
装完**重启 Claude Code**，任意目录下 `claude mcp list` 或 `/mcp` 都能看到 `reconbridge`（25 个工具），并会随装一个 `reconbridge` skill（`~/.claude/skills/reconbridge/`，逆向类任务时新会话自动加载工作流与踩坑清单）。
